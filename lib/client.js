...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DOT_COLOR = { ok: "var(--dsw-alias-state-success-primary,#4ade80)", warn: "#f5a623", over: "#f87171" };

		/* ── styles ───────────────────────────────────────────────────── */
		/* Neutral gray theme (no blue, no pure black): grayscale only,
		   with green/amber/red kept solely for the budget dot and hints. */
		const css = `.${CSS_NS}_root{align-items:center;gap:6px;display:inline-flex}
.${CSS_NS}_chip{box-sizing:border-box;height:22px;cursor:pointer;color:#d1d5db;background:#262626;border:1px solid #525252;border-radius:11px;align-items:center;gap:5px;padding:0 10px;font-size:12px;line-height:18px;display:inline-flex;font-variant-numeric:tabular-nums}
.${CSS_NS}_chip:hover{color:#f9fafb;border-color:#d1d5db;background:#2f2f2f}
.${CSS_NS}_chip[data-open="true"]{color:#f9fafb;border-color:#d1d5db;background:#2f2f2f}
.${CSS_NS}_dot{width:6px;height:6px;border-radius:50%;flex:none}
.${CSS_NS}_dotOver{animation:${CSS_NS}_pulse 1.6s ease-in-out infinite}
@keyframes ${CSS_NS}_pulse{0%,100%{opacity:1}50%{opacity:.35}}
.${CSS_NS}_badge{color:#f87171;font-weight:600}
.${CSS_NS}_pop{position:fixed;z-index:2147483000;transform:translateY(-100%);width:360px;max-width:calc(100vw - 24px);max-height:70vh;overflow:auto;box-sizing:border-box;background:#1f1f1f;border:1px solid #4b5563;border-radius:12px;box-shadow:0 14px 44px rgba(0,0,0,.55);padding:12px 14px;color:#d1d5db;font:12px/1.6 -apple-system,"Segoe UI",Roboto,"PingFang SC","Microsoft YaHei",sans-serif}
.${CSS_NS}_head{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px}
.${CSS_NS}_title{font:600 13px/1.5 ui-monospace,Consolas,monospace;color:#f3f4f6;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.${CSS_NS}_big{font:600 15px/1.5 ui-monospace,Consolas,monospace;color:#f3f4f6;flex:none}
...
.${CSS_NS}_v{flex:1;text-align:right;color:#d1d5db;font-variant-numeric:tabular-nums;font:11px/1.6 ui-monospace,Consolas,monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.${CSS_NS}_hint{color:#9ca3af;font-size:11px;margin-top:8px;border-top:1px solid #3f3f46;padding-top:6px}
.${CSS_NS}_hintWarn{color:#f5a623;font-size:11px;margin-top:8px;border-top:1px solid #3f3f46;padding-top:6px}
.${CSS_NS}_hintOver{color:#f87171;font-size:11px;margin-top:8px;border-top:1px solid #3f3f46;padding-top:6px}
.${CSS_NS}_btns{display:flex;gap:8px;margin-top:10px}
.${CSS_NS}_btn{box-sizing:border-box;height:28px;flex:none;cursor:pointer;color:#e5e7eb;background:transparent;border:1px solid #525252;border-radius:14px;padding:0 12px;font:inherit;font-size:12px;line-height:18px}
...
.${CSS_NS}_thrRow{align-items:center;gap:8px;display:flex;margin-top:10px;border-top:1px solid #3f3f46;padding-top:8px}
.${CSS_NS}_lbl{color:inherit;opacity:.8;font-size:12px;line-height:18px;flex:none}
.${CSS_NS}_inp{box-sizing:border-box;color:#e5e7eb;background:transparent;border:1px solid #525252;border-radius:9px;padding:4px 8px;font:inherit;font-size:12px;line-height:18px;outline:none;width:110px;flex:none}
.${CSS_NS}_inp:focus{border-color:#d1d5db}
.${CSS_NS}_err{color:#f87171;font-size:11px;margin-top:4px}
...
.${CSS_NS}_saved{color:#d1d5db;font-size:11px}`;
		...
		function ContextLine({ useProjection, t, sessions, workspaces, sessionId, inputActions }) {
			...
			const dotCls = band === "over" ? CSS_NS + "_dot " + CSS_NS + "_dotOver" : CSS_NS + "_dot";
			...
			return jsxs(Fragment, {
				children: [
					jsxs("button", {
						...
						onClick: toggle,
						children: [
							jsx("span", { className: dotCls, style: { background: DOT_COLOR[band] } }),
